...
### Positional encoding
作者发现网络如果直接输入xyzθφ 坐标，训练表示 high-frequency 的颜色和几何形状渲染效果会非常不好。这和Rahaman的论文中描述得一致，原因是神经网络会倾向于学习 low-frequency 的函数。他们也证明了在把输入放入网络中之前，把输入映射到高维空间然后再输入到网络中，网络能够更好的拟合包含 high-frequency 函数的数据。
